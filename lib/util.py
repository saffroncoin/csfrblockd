...
def call_jsonrpc_api(method, params=None, endpoint=None, auth=None, abort_on_error=False):
    if not endpoint: endpoint = config.CSFRD_RPC
    if not auth: auth = config.CSFRD_AUTH
...
def decorate_message_for_feed(msg, msg_data=None):
    """This function takes a message from csfrd's message feed and mutates it a bit to be suitable to be
    sent through the csfrblockd message feed to an end-client"""
    if not msg_data:
...
def is_caught_up_well_enough_for_government_work():
    """We don't want to give users 525 errors or login errors if csfrblockd/csfrd is in the process of
    getting caught up, but we DO if csfrblockd is either clearly out of date with the blockchain, or reinitializing its database"""
...
def block_cache(func):
    def cached_function(*args, **kwargs):
        ...
        cached_result = config.mongo_db.csfrblockd_cache.find_one({'block_index': block_index, 'function': function_signature})
        ...
        if not cached_result or config.TESTNET:
            ...
            try:
                ...
                config.mongo_db.csfrblockd_cache.insert({
...
def clean_block_cache(block_index):
    ...
    config.mongo_db.csfrblockd_cache.remove({'block_index': {'$lt': block_index}})
