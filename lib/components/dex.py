...
def get_pairs_with_orders(addresses=[], max_pairs=12):
    ...
    for my_pair in my_pairs:
        ...
        if my_pair['pair'] == 'SFR/cSFR': # cSFR/SFR always in first
...
def get_pairs(quote_asset='cSFR', exclude_pairs=[], max_pairs=12, from_time=None):
...
def get_quotation_pairs(exclude_pairs=[], max_pairs=12, from_time=None, include_currencies=[]):
    ...
    for currency in currencies:
        ...
        for currency_pair in currency_pairs:
            if currency_pair['pair'] == 'cSFR/SFR':
    ...
    for currency in config.MARKET_LIST_QUOTE_ASSETS:
        if len(top_pairs) < max_pairs:
            ...
            for currency_pair in currency_pairs:
                ...
                if currency_pair['pair'] == 'cSFR/SFR': # cSFR/SFR always in first
                    top_pairs.insert(0, top_pair)
                else:
                    top_pairs.append(top_pair)
                all_assets += [currency_pair['base_asset'], currency_pair['quote_asset']]

    if 'cSFR/SFR' not in [p['base_asset'] + '/' + p['quote_asset'] for p in top_pairs]:
        top_pairs.insert(0, {
            'base_asset': 'cSFR',
            'quote_asset': 'SFR'
        })
...
def get_assets_supply(assets=[]):
    ...
    if 'cSFR' in assets:
        supplies['cSFR'] = (util.call_jsonrpc_api('get_xcp_supply', [])['result'], True)
        assets.remove('cSFR')
...
def get_markets_list(mongo_db=None, quote_asset=None, order_by=None):
    ...
    for pair in pairs:
        ...
        if market['base_asset'] == 'cSFR' and market['quote_asset'] == 'SFR':
