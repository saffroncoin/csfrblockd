...
if __name__ == '__main__':
    ...
    parser.add_argument('--csfrd-rpc-connect', help='the hostname of the csfrd JSON-RPC server')
    parser.add_argument('--csfrd-rpc-port', type=int, help='the port used to communicate with csfrd over JSON-RPC')
    parser.add_argument('--csfrd-rpc-user', help='the username used to communicate with csfrd over JSON-RPC')
    parser.add_argument('--csfrd-rpc-password', help='the password used to communicate with csfrd over JSON-RPC')
    ...
    parser.add_argument('--rpc-port', type=int, help='port on which to provide the csfrblockd JSON-RPC API')
    parser.add_argument('--rpc-allow-cors', action='store_true', default=True, help='Allow ajax cross domain request')
    parser.add_argument('--socketio-host', help='the interface on which to host the csfrblockd socket.io API')
    parser.add_argument('--socketio-port', type=int, help='port on which to provide the csfrblockd socket.io API')
    parser.add_argument('--socketio-chat-host', help='the interface on which to host the csfrblockd socket.io chat API')
    parser.add_argument('--socketio-chat-port', type=int, help='port on which to provide the csfrblockd socket.io chat API')
    ...
    if not args.data_dir:
        config.DATA_DIR = appdirs.user_data_dir(appauthor='SFRDirect', appname='csfrblockd', roaming=True)
    else:
    ...
    config_path = os.path.join(config.DATA_DIR, 'csfrblockd.conf')
    ...
    if args.pid_file:
        ...
        config.PID = os.path.join(config.DATA_DIR, 'csfrblockd.pid')
