...
By default, ``csfrblockd`` will listen on port ``4001`` for API
...
An individual cSFRwallet user needs a way to identify themselves to ``csfrblockd`` for things like storing
and retrieving their wallet preferences data, and more.

For this purpose, we define the concept of a wallet ID, which is simply the user's cSFRwallet 12-word password,
...
An object that stores the cSFRwallet preferences for the given wallet ID.
...
* **address_aliases** (*list*): A list of zero or objects, with each object having an ``address`` string property,
  being the Saffroncoin base56 address, and an ``alias`` string property, being the textual alias (i.e. nickname)
